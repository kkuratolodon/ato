...
// Setup the routes with auth middleware
...
Given('a purchase order {string} exists with {string} status', async (poId, status) => {
  ...
  sinon.stub(purchaseOrderService, 'getPurchaseOrderStatus').resolves({
    id: poId,
    status: statusEnum
  });
  
  sinon.stub(purchaseOrderService, 'getPartnerId').resolves('partner-123');
...
Given('the purchase order {string} does not exist', async (poId) => {
  ...
  sinon.stub(purchaseOrderService, 'getPartnerId').rejects(error);
  sinon.stub(purchaseOrderService, 'getPurchaseOrderStatus').rejects(error);
...
Given('the purchase order {string} belongs to another user', async (poId) => {
  ...
  sinon.stub(purchaseOrderService, 'getPartnerId').resolves('other-partner-id');
